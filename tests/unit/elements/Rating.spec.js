...
describe('Rating.vue', () => {
  ...
  it('emits the correct value', () => {
    ...
    wrapper.findAll('.icon.unit').at(VALUE - 1)
      .trigger('click')
  ...
  it('sets icons as expected', () => {
    ...
    const wrapper = getWrapper()
    expect(wrapper.findAll(`*[icon]`).length)
      // 4 because there is a default icon, take a look at Rating.vue
      .toBe(4)
    expect(wrapper.findAll(`*[icon*="${ICON_SET[0]}"]`).length)
      .toBe(1)
    expect(wrapper.findAll(`*[icon*="${ICON_SET[1]}"]`).length)
      .toBe(1)
    expect(wrapper.findAll(`*[icon*="${ICON_SET[2]}"]`).length)
      .toBe(1)
  })
  ...
  it('doesnt emit on disabled mode ', () => {
    ...
    wrapper.findAll('.icon.unit').at(3)
      .trigger('click')
